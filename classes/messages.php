...
class Messages {
	function list_thread($thread_id) {
		...
		while($row_message = $message_result->fetch_array()) {
			...
			if($row_message['from_id'] == $_SESSION['user_id'] || $row_message['to_id'] == $_SESSION['user_id']) { 
				if($row_message['from_id'] == $_SESSION['user_id']) { 
					$sender = $label_obj->label("sender_me");
				} else { 
				...
				$content .= '<hr />
 						       <div class="col-sm-3">';				
				$content .= '<h5>'.$label_obj->label("sender_from").': <strong>'.$sender.'</strong></h5>';
				$content .= '<h5>'.$label_obj->label("sender_to").': <strong>'.$receiver.'</strong></h5>';
		...
		$content .= '<div class="form-group">
					<textarea name="reply_detail" class="tinyst form-control" placeholder="'.$label_obj->label("send_a_reply").'"></textarea>
					</div>';
	...
	function send_reply($reply_to, $subject_id, $reply_detail) { 
			...
			return $label_obj->label("message_sent_success");
	...
	function new_message($username_email, $subject, $message) { 
		...
		if($num_rows >0) {
			...
			return $label_obj->label("no_user_to_message").' '.$username_email;
		}
	}//new_message ends here.
	...
	function level_message($level_name, $subject, $message) { 
		...
		return $label_obj->label("message_to_all").' '.$level_name.'.';
	...
	function message_all($subject, $message) { 
		...
		return $label_obj->label("message_sent_to_all");
	}//message to all admins ends here.
