...
class Notes {
	...
	function update_note($note_id, $note_title, $note_detail) { 
		...
		$result = $db->query($query) or die($db->error);
		return $label_obj->label("note_update_success");
	...
	function list_notes() {
		...
		while($row = $result->fetch_array()) { 
			...
			$content .= '<input type="submit" class="btn btn-default btn-sm pull-left" value="'.$label_obj->label('edit').'">';
			$content .= '</form>';
			...
			$content .= '<input type="submit" class="btn btn-default btn-sm pull-right" value="'.$label_obj->label('delete').'">';
			$content .= '</form>';
            $content .= '</div><div class="clearfix"></div>';
            $content .= '</div>';
			$content .= '<p><strong>'.$label_obj->label("date").': </strong>'.$note_date.'</p>';
	...
	function add_note($note_title, $note_detail) { 
		...
		$result = $db->query($query) or die($db->error);
		return $label_obj->label("note_added_success");
	}//add notes ends here.
	...
	function delete_note($note_id) {
		...
		global $label_obj;
			...
			$result = $db->query($query) or die($db->error);
			$message = $label_obj->label("note_delete_success");	
		return $message;
