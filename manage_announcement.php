<?php
	...
	if(isset($_POST['add_announcement']) && $_POST['add_announcement'] == '1') { 
		...
		if($announcement_title == '') { 
			$message = $label_obj->label("announcement_title_required");
		} else if($announcement_detail == '') { 
			$message = $label_obj->label("announcement_detail_required");
		}  else {
	...
	if(isset($_POST['edit_announcement'])){ $page_title = $label_obj->label('edit_announcement'); } else { $page_title = $label_obj->label('add_announcement');} //page title set.
...
?>
    ...
    <form action="<?php $_SERVER['PHP_SELF']?>" id="add_user" name="user" method="post" enctype="multipart/form-data" role="form">
     ...
     <label class="control-label"><?php echo $label_obj->label("announcement_title"); ?>*</label>
     <input type="text" class="form-control" name="announcement_title" required="required" placeholder="<?php echo $label_obj->label("announcement_title"); ?>" value="<?php echo $announcement_obj->announcement_title; ?>" />
     </div>
        
        <div class="form-group">
        <label class="control-label"><?php echo $label_obj->label("announcement_detail"); ?>*</label>
        <textarea name="announcement_detail" class="tinyst form-control" placeholder="<?php echo $label_obj->label("announcement_detail"); ?>"><?php echo $announcement_obj->announcement_detail; ?></textarea>
        </div>
        ...
        <div class="form-group">
        <label class="control-label"><?php echo $label_obj->label("announcement_status"); ?></label>
      ...
      <?php 
        ...
        <div class="form-group">
            <input type="submit" value="<?php if(isset($_POST['edit_announcement'])){ echo $label_obj->label("edit_announcement"); } else { echo $label_obj->label("add_announcement");} ?>" class="btn btn-primary" />
        </div>
    </form>
