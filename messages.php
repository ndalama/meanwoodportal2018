<?php
	...
	if(isset($_POST['reply_form']) && $_POST['reply_form'] == '1') {
		...
		$message = $label_obj->label("message_is_empty");
		}
	}
	
	$page_title = $label_obj->label("my_messages"); //You can edit this to change your page title.
	$sub_title = "Manage your messages";
	require_once("includes/header.php"); //including header file.

?>
<!--row starts here.-->
<div class="row" style="opacity: 0.9; background: #fff; padding: 20px;">
  ...
  <div class="modal-dialog">
	<div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><?php echo $label_obj->label("send_message"); ?></h4>
      </div>
	  <div class="modal-body">
           ...
           <form class="form-horizontal" data-async data-target="#rating-modal" method="POST" enctype="multipart/form-data" role="form">
      		<div class="form-group">
				<label class="control-label"><?php echo $label_obj->label("message_to"); ?> <small><?php echo $label_obj->label("email_or_username"); ?></small></label>
				<input type="text" class="form-control" name="message_to" required="required" value="" />
			</div>
			
			<div class="form-group">
				<label class="control-label"><?php echo $label_obj->label("subject"); ?></label>
        ...
        <select name="subject" class="form-control" required="required">
          ...
          <option value="Credit">Credit</option>
			...
			<div class="form-group">
				<label class="control-label"><?php echo $label_obj->label("message"); ?></label>
              ...
              <input type="submit" id="submit" value="<?php echo $label_obj->label("send_message"); ?>" class="btn btn-primary" />
           </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo $label_obj->label("close"); ?></button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
  		<hr />
        <a href="messages.php" class="btn btn-<?php if(isset($_GET['type'])){echo'default';}else{echo'primary';}?>" style="width:100%"><?php echo $label_obj->label("inbox"); ?></a>
        <div style="height:7px;"></div>
        <a href="messages.php?type=sent" class="btn btn-<?php if(isset($_GET['type'])){echo'primary';}else{echo'default';}?>" style="width:100%"><?php echo $label_obj->label("sent"); ?></a>
  </div>
